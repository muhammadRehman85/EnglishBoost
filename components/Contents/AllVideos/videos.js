...
const Videos = () => {
  ...
  return (
    <ScrollView style={styles.videoWrapper}>
      <View style={{width:'95%',backgroundColor:'yellow',height:300,marginLeft:10}}><Text>Google Ad</Text></View>
      ...
      {DATA.map((item) => (
        <View key={item.key}  style={styles.videoContainer}>
          <YoutubePlayer height={ 300} videoId={item.videoId} play={false} style={styles.youtube} showinfo={false} modestbranding />
...
const styles = StyleSheet.create({
  ...
  title: { fontWeight: "bold", fontSize: 22, color: "#8870FF",marginBottom:20 },
